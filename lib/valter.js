...
var Valter = (function () {
    ...
    function Valter (isSilent) {
        ...
        this.readAsJson= function(path) {
            if (!path) { throw path + " argument is mandatory!"; }
            return JSON.parse(fs.readFileSync(path, 'utf8'));
        };
        
        this.isValidContract=function(contract){

                if(!contract.hasOwnProperty("expect")){ 
                    self.errorOccured("STATUS : the contract has no 'expect' property!");
                    self.defaultState.keepAlive = false;
                    return;
                }
                if(!contract.expect.hasOwnProperty("body") || !contract.expect.hasOwnProperty("request")){ 
                    self.errorOccured("STATUS : the contract has no 'body' or 'request' property!");
                    self.defaultState.keepAlive = false;
                    return;
                }
        };
        ...
        return {
            ...
            checkContract: function(givenContract) {
                var that = this;
                ...
                var contractPath = self.defaultState.contractPathList.pop();
                self.log("Contract check for: " + contractPath);
                var contract = givenContract || self.readAsJson(contractPath);
                ...
                request(serviceUrl, options, function(error, response, body) {
                    ...
                    if (!error && response.statusCode == 200) {
                        ...
                        if(contract.expect.request.status_code === response.statusCode && !self.defaultState.hasError){
                            self.log("STATUS: OK\n");
                        }
                        else{
                            self.log("Contract statuscode does not match!\n");
                            self.defaultState.hasError = true;
                            self.defaultState.keepAlive = false;
                            return;
                        }
                    }
